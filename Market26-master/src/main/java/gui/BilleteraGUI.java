...
public class BilleteraGUI extends JFrame {
    ...
    public BilleteraGUI(String email) {
        ...
        getContentPane().setBackground(Color.WHITE); // Fondo blanco más limpio
        ...
        btnRecarga.setBackground(new Color(200, 255, 200));
        add(btnRecarga);

        JButton btnRetirar = new JButton("Retirar");
        btnRetirar.setBounds(150, 60, 120, 40);
        btnRetirar.setBackground(new Color(255, 200, 200));
        add(btnRetirar);
        ...
        btnVer.addActionListener(e -> {
            try {
                // 1. Cargamos el historial solo para el texto
                ...
                // 2. Pedimos el saldo REAL a la base de datos (evita fallos de cálculo)
                float saldoReal = MainGUI.getBusinessLogic().getSaldoUsuario(email);
                lblSaldoValor.setText(String.format("%.2f€", saldoReal));
                
                // 3. Sincronizamos la pantalla principal
                notificarMainGUI();
    ...
    // MÉTODO MÁGICO: Busca la ventana MainGUI abierta y la actualiza al vuelo
    private void notificarMainGUI() {
        for (Frame frame : Frame.getFrames()) {
            if (frame instanceof MainGUI) {
                ((MainGUI) frame).actualizarSaldo();
            }
        }
    }
}